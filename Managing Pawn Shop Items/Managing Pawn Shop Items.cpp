...
int main(){
    SortList<pair<double, string>> list;
    int n;
    cin >> n;
    // n commands
    for(int i = 1; i<=n; ++i){
        ...
        if(s == "ADD"){
            ...
            getline(cin, title);
            // add
            list.add(make_pair(price, title));
        }
        ...
        if(s == "REMOVE"){
            ...
            getline(cin, title);
            //find index of item
            int index = list.indexOf(make_pair(price,title));
            // if exist: remove
            if(index != -1)
                list.remove(index);
        }
        ...
        if(s == "LIST"){
            // get data
            char cy1, cy2;
            pair<double, string> price_from, price_to;
            cin >> cy1 >> price_from.first >> cy2 >> price_to.first;
            price_from.second = "";
            price_to.second = "z";
            SortList<pair<double, string>> res;
            //res = list of items in defined range
            res = list.searchRange(price_from, price_to);
            // if items exists
            if(!res.isEmpty()){
                //print list of item in defined range
                pair<double,string> ans;
                for(int i = 1 ; i < res.size(); ++i){
                    ans = res.get(i);

                    cout << fixed << setprecision(2) <<cy1 << ans.first << " "<< ans.second << ", ";
                }
                ans = res.get(res.sjze);
                cout << fixed << setprecision(2) <<cy1 << ans.first << " "<< ans.second <<"\n";
            }
            else
                cout << "\n";
